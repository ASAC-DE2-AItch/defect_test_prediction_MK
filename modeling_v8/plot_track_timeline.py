#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""modeling_v8 — 관찰 트랙(F-T15 · F-P3)별 valid/test 전체 기간 예측 개형 플롯.

노트북 변수에 의존하지 않는 독립 스크립트다. v8 피처 빌더(Cell 1·3)를 그대로 이식해
피처 테이블을 만든 뒤, 각 트랙을 **LightGBM**(M8_PARAMS, full-train 705 rounds)으로
학습·예측하고, 트랙마다 valid(위)/test(아래) 2-패널 타임라인 개형을 그린다.
  ...
  출력:
    outputs/timeline_F-T15.png , outputs/timeline_F-P3.png

사용법: 이 파일을 modeling_v8/ 폴더에 두고
    python plot_track_timeline.py            # F-T15, F-P3 모두
    python plot_track_timeline.py F-T15      # 특정 트랙만
"""
import sys
import json
...
# ── LightGBM 파라미터 (Cell 7 M8_PARAMS = 복원 pkl 값) ──────
...
def build_features(df_raw, pm_events):
    ...
    return res  # '_date'(WF 대표 시각, KST) 컬럼을 x축·C6_1 배치용으로 보존
...
def split_frame(Xdf, feats, model, ans_path):
    ...
    d['pred'] = model.predict(Xdf[feats])
...
def main():
    ...
    for tname in want:
        ...
        dv = split_frame(Xva, feats, model, ANS / "valid_Y_answer.csv")
        dt = split_frame(Xte, feats, model, ANS / "test_Y_answer.csv")
        ...
        fig.suptitle(f"modeling_v8 · 트랙 {tname} (LightGBM) 전체 기간 예측 개형 (valid / test)",
                     fontsize=15, fontweight="bold", y=0.98)
        plt.tight_layout(rect=[0, 0, 1, 0.97])
        out = OUT / f"timeline_{tname}.png"
