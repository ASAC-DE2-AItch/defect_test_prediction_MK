#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""modeling_v8 — 관찰 트랙(F-T15 · F-P3)별 valid/test 전체 기간 예측 개형 플롯 (ExtraTrees 버전).

plot_track_timeline.py 의 ExtraTrees(ET) 판이다. v8 피처 빌더(Cell 1·3)를 이식해 피처
테이블을 만든 뒤, 각 트랙을 **ExtraTrees**(Cell 15 방식: train에서만 median 대치 fit →
ExtraTreesRegressor 500그루)로 전체학습·예측하고, 트랙마다 valid(위)/test(아래) 2-패널
타임라인 개형을 그린다. ET는 M5a 챔피언(F-C10 CV 38.00·valid 37.12·test 36.89).

  입력(모두 modeling_v8/ 기준 상대경로):
    ../문제1(하)/{train_data,valid_X,test_X}.csv   피처 빌드·학습
    ../pm_log.json                                  레짐(verdict) 정본
    ../문제1_하_answer/{valid,test}_Y_answer.csv    RMSE 채점 + 개별 WF 실측
  출력:
    outputs/timeline_ET_F-T15.png , outputs/timeline_ET_F-P3.png

사용법: 이 파일을 modeling_v8/ 폴더에 두고
    python plot_track_timeline_et.py           # F-T15, F-P3 모두
    python plot_track_timeline_et.py F-P3       # 특정 트랙만
"""
import sys
import json
...
TRACKS = {
    ...
    # 'F-C10': CORE10,   # 원하면 주석 해제 — ET 챔피언 트랙
}

# ── ExtraTrees 설정 (Cell 15 _fp_et / et_valid_test 동일) ───
ET_N_ESTIMATORS = 500
ET_RANDOM_STATE = 42
...
def build_features(df_raw, pm_events):
    ...
    return res  # '_date'(WF 대표 시각, KST) 컬럼 보존
...
def fit_et(Xtr, feats, y):
    """Cell 15 et_valid_test와 동일: median 대치는 train에서만 fit(누수 없음) → ET 500그루."""
...
def draw_panel(ax, d, name):
    ...
    ax.plot(r.index, r["pred"], c="tab:green", lw=1.4, label="예측 (12h 평균)", zorder=3)
...
def split_frame(Xdf, feats, predict_fn, ans_path):
    """트랙 예측 + 정답 + 시각/배치를 WF 단위로 결합."""
...
def main():
    ...
    Xtr = build_features(pd.read_csv(DATA / "train_data.csv"), pm_events)
    Xva = build_features(pd.read_csv(DATA / "valid_X.csv"),   pm_events)
    Xte = build_features(pd.read_csv(DATA / "test_X.csv"),    pm_events)
    y = Xtr[TARGET_COL].to_numpy(float)
    print(f"피처 테이블: train {Xtr.shape} / valid {Xva.shape} / test {Xte.shape}")

    for tname in want:
        feats = TRACKS[tname]
        miss = [c for c in feats if c not in Xtr.columns]
        assert not miss, f"{tname} 피처 누락: {miss}"
        predict_fn = fit_et(Xtr, feats, y)

        dv = split_frame(Xva, feats, predict_fn, ANS / "valid_Y_answer.csv")
        dt = split_frame(Xte, feats, predict_fn, ANS / "test_Y_answer.csv")

        fig, axes = plt.subplots(2, 1, figsize=(16, 10), sharex=True)
        r_v = draw_panel(axes[0], dv, "valid")
        r_t = draw_panel(axes[1], dt, "test")
        axes[1].set_xlabel("날짜")
        axes[1].xaxis.set_major_formatter(mdates.DateFormatter("%m-%d"))
        axes[1].xaxis.set_major_locator(mdates.WeekdayLocator(byweekday=mdates.MO))
        fig.suptitle(f"modeling_v8 · 트랙 {tname} (ExtraTrees) 전체 기간 예측 개형 (valid / test)",
                     fontsize=15, fontweight="bold", y=0.98)
        plt.tight_layout(rect=[0, 0, 1, 0.97])
        out = OUT / f"timeline_ET_{tname}.png"
        plt.savefig(out, dpi=130, bbox_inches="tight"); plt.close(fig)
        print(f"[{tname}] valid RMSE {r_v:.2f} | test RMSE {r_t:.2f}  → {out}")
