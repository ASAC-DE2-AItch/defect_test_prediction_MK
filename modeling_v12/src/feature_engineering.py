...
def _parse_pm_log(pm_log: list) -> list[tuple]:
    """pm_log 항목을 (Timestamp, type) 목록으로 정규화한다.

    ⚠️ 기입 규약 (2026-07-09, PM 측 확정): **Qual 요란(loud) 판정이 난 PM만 기입한다.**
      - 요란 PM = 챔버 교란 → C65 급등 후 감쇠 (12-24 실측 +486 급등이 요란 확정)
      - 조용(quiet) PM = 챔버 상태 무영향 → 로그에 아예 안 적음
        → 플래그 유지 + days 리셋 안 됨 = 의도된 동작.
        조용 PM의 흔적은 C33(장비 카운터, 모든 개방마다 리셋)이 센서로 자동 공급.
    이 규약 하에서 로그의 모든 항목은 요란 PM이므로 type 필드는 잉여다
    (하위 호환으로 파싱만 유지). 두 형식 모두 지원:
      레거시: ["2018-12-24"] / 신규: [{"date": "2018-12-24", "type": "major"}]
    """
    out = []
    for e in pm_log:
...
def _compute_time_features(
    ...
    days_since_last_pm: 가장 최근 (요란) PM으로부터 경과일 — 과도 감쇠 곡선의 위치.
                        로그 이전 구간은 캠페인 시작일(_REF_DATE) 기준으로 경과일 계산.
                        요란 PM이 기입될 때마다 0으로 리셋. (조용 PM은 로그에 없으므로
                        리셋하지 않음 — 의도된 동작. 개방 단위 시계는 C33이 담당)
    pm_cycle_count:     지금까지 기입된 (요란) PM 횟수 (메타데이터, 모델 미사용).
    is_post_loud_pm:    "요란 PM 이후(과도 구간일 수 있음)인가" (0/1).
                        1 = 로그에 요란 PM이 존재 (급등→감쇠 곡선 적용)
                        0 = 로그에 요란 PM 없음 = "위상 미상의 안정 상태" (pre 구간)
                        ⚠️ 한 번 1이 되면 0으로 돌아가지 않는 one-way 플래그.
                        '얼마나 식었나'는 이 플래그가 아니라 days(연속값)가 담당한다.
                        (구명: is_post_pm → is_high_regime → is_post_loud_pm.
                         새 기입 규약에선 셋 다 영구 동치라 한 벌로 통합, 2026-07-09)
    """
    events = _parse_pm_log(pm_log)
    ...
    for d in wf_dates:
        ...
        if past:
            last_pm, _last_type = past[-1]
            days_vals.append((d - last_pm).total_seconds() / 86400)
            cycle_vals.append(len(past))
            loud_vals.append(1)   # 규약상 로그의 모든 PM = 요란
        else:
            # 요란 PM 기록 이전: 캠페인 시작일(_REF_DATE) 기준 경과일 (안정 꼬리 트렌드 포착)
...
def make_meta_features(
    ...
    meta = meta.merge(
        wf_date[[ID_COL, 'days_since_last_pm', 'pm_cycle_count', 'is_post_loud_pm', 'hour']],
        on=ID_COL,
    )

    # --- 레짐 플래그 정리 (2026-07-09 A안) ---
    #   is_post_loud_pm이 유일한 레짐 플래그. 새 기입 규약(요란 PM만 기입) 하에서
    #   구 is_post_pm(PM 존재 여부)·구 is_high_regime(마지막 PM==major)과 영구 동치라
    #   이중 컬럼을 한 벌로 통합했다. pm_cycle_count(숫자)는 미학습 값 위험으로
    #   모델 피처로 쓰지 않고 메타데이터로만 유지.

    # --- 교호(interaction) 피처 ---
    #   시간 관련 상호작용이 C65 예측에 강한 신호를 준다 (실험으로 검증, -4.6 RMSE).
    #   구성 요소가 모두 프로덕션-안전 피처(하드코딩 날짜 없음)라 PM 반복에도 확장된다.
    #   dslp_x_hour : 요란 PM 이후 경과에 따라 3교대(hour) 패턴의 효과가 달라지는 정도
    #   hour_x_c33  : 장비 시간 카운터(C33) 상태와 측정 시각의 복합 효과
    #   post_pm_days: 요란 PM 이후(과도 구간) 경과일만 남기는 '마스크' (로그 이전=0).
    #                 곱셈이 아니라 0/1 게이팅이므로 PM이 반복돼도 경과일이 왜곡되지 않는다.
    #                 (구 high_regime_days와 새 규약 하 영구 동치 → post_pm_days로 통합)
    meta['dslp_x_hour']  = meta['days_since_last_pm'] * meta['hour']
    meta['hour_x_c33']   = meta['hour']              * meta['C33']
    meta['post_pm_days'] = meta['days_since_last_pm'] * meta['is_post_loud_pm']
...
def run() -> pd.DataFrame:
    ...
    # 알려진 PM 이벤트 목록 (프로덕션에서는 PM 발생 시 항목 추가).
    # 신규 형식: {"date", "type"(major/minor)}. 12-24 PM은 직후 C65 +486 급등이
    # 관측됐으므로 '메이저'(큰 정비 → 스파이크). 마이너 PM(작은 정비, 예: 데이터 끝
    # 이후 cycle 2 시작)은 시뮬레이션 시점에 추가한다.
    pm_log = [{"date": PM_SHIFT_DATE, "type": "major"}]
